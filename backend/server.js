...
app.get("/adminanalysis", (req, res) => {
  res.sendFile(path.join(__dirname, "../views/adminanalysis.html"));
});
...
//login otp verify
api.post("/userverifylogin", async (req, res) => {
  try {
...
api.post("/onboarduser", async (req, res) => {
  try {
    const {
      username,
      userroom,
...
api.post("/authenticate", authenticateToken, async (req, res) => {
  return res.json({ data: "ok" });
});

//delete member------------

api.post("/deleteMember", async (req, res) => {
  try {
    const { _id } = req.body;
...
api.post("/editmember", async (req, res) => {
  try {
    const { _id } = req.body;
...
api.post("/namesearch", async (req, res) => {
  try {
...
api.get("/vacatehistory", async (req, res) => {
  try {
...
//user signup
api.post("/usersignup", async (req, res) => {
  try {
    ...
    const usersignupexist = await signupuser.find({
...
//users login route
api.post("/userlogin", async (req, res) => {
  try {
    const { mobilenumber, password } = req.body;
...
api.get("/mystaydays", authenticateToken, async (req, res) => {
  try {
...
api.get("/mypaymentstatus", authenticateToken, async (req, res) => {
  try {
    const phone = req.user.mobilenumber;
    const now = new Date();
    const months = [
...
api.get("/myprofile", authenticateToken, async (req, res) => {
  try {
    const phone = req.user.mobilenumber;
    const u = await user.findOne({ PhoneNumber: phone });
    res.json(u);
...
api.get("/mypayments", authenticateToken, async (req, res) => {
  try {
    const phone = req.user.mobilenumber;
...
api.post("/addpayment", async (req, res) => {
  try {
    const {
      paymentphone,
      paymentmonth,
...
//fetching payments
api.post("/fetchpayments", async (req, res) => {
  try {
    const { fetchmonth } = req.body;
    const paymentslist = await payment
      .find({ Month: fetchmonth }, { __v: 0 })
      .sort({ Date: -1 });
...
api.post("/paymenthistory", async (req, res) => {
  try {
...
api.post("/roommembers", async (req, res) => {
  try {
    const { roomno } = req.body;
    const members = await user
      .find({ Room: roomno }, { __v: 0 })
      .sort({ Name: 1 });
...
api.post("/daywisemembers", async (req, res) => {
  try {
...
api.post("/pendingmembers", async (req, res) => {
  try {
    const { pendingmonth } = req.body;
    const paidphones = await payment.distinct("PhoneNumber", {
      Month: pendingmonth,
    });
...
//payment list
api.get("/paymentslist", authenticateToken, async (req, res) => {
...
api.get("/roommates", authenticateToken, async (req, res) => {
  const phone = req.user.mobilenumber;
...
api.get("/download", (req, res) => {
  try {
     const filePath = path.join(__dirname, "routes", "menuhostel.pdf");
